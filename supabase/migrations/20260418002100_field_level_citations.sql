-- ============================================================
-- 20260418002100_field_level_citations.sql
--
-- Closes the field-level citation gap found in Phase 6 QA:
-- the import pipeline previously stored all citations at the
-- problem_template level only.  This migration enables per-row
-- citation storage for requirements and pilot_frameworks.
--
-- Changes:
--   1. Extend content_citations.target_type check to include
--      'pilot_framework' (requirement was already allowed).
--   2. Add public RLS policies so visitors can read citations
--      for published requirements and pilot_frameworks.
--   3. Add cascade-cleanup trigger for pilot_framework deletions
--      (requirement cleanup trigger already exists from migration
--       20260418002000).
--   4. Rebuild import_curated_problem_v1 to insert field-level
--      citations after each requirement/pilot_framework row.
-- ============================================================

-- ── 1. Extend target_type check constraint ───────────────────

alter table public.content_citations
  drop constraint if exists content_citations_target_type_check;

alter table public.content_citations
  add constraint content_citations_target_type_check
...
-- ── 2. Public RLS policies for sub-row citations ─────────────

-- Requirement citations: visible when the parent problem is published.
create policy "public_reads_published_requirement_citations"
  ...
  using (
    ...
    and exists (
      ...
      join public.problem_templates pt on pt.id = r.problem_id
      where r.id    = content_citations.target_id
        and pt.status = 'published'
    )
  );

-- Pilot-framework citations: visible when the parent problem is published.
create policy "public_reads_published_pilot_framework_citations"
  on public.content_citations
  for select
  using (
    target_type = 'pilot_framework'
    and exists (
      select 1
      from public.pilot_frameworks pf
      join public.problem_templates pt on pt.id = pf.problem_id
      where pf.id   = content_citations.target_id
        and pt.status = 'published'
    )
  );

-- ── 3. Cleanup trigger for pilot_framework deletions ─────────

drop trigger if exists cleanup_pilot_framework_citations
  on public.pilot_frameworks;

create trigger cleanup_pilot_framework_citations
  after delete on public.pilot_frameworks
  for each row
  execute function public.delete_content_citations_for_target('pilot_framework');

-- ── 4. Rebuild import_curated_problem_v1 ─────────────────────
--
-- Additions vs the previous version:
--   • RETURNING id on both the requirement INSERT and the
--     pilot_framework INSERT so we can attach per-row citations.
--   • A second citation loop inside the requirements loop that
--     writes content_citations rows with target_type='requirement'.
--   • A citation loop after the pilot_framework insert that
--     writes content_citations rows with target_type='pilot_framework'.
--
-- Payload shape (new fields, both optional – old payloads still work):
--   requirements[n].citations  – array of citation objects for that req
--   pilot_framework.citations  – array of citation objects for the pf row
...
declare
  ...
  v_pf_id               uuid;
  v_citation            jsonb;
  v_field_citation      jsonb;
  v_access_date         date;
  v_access_date_raw     text;
begin
  ...
  -- Requirements: insert row then attach field-level citations.
  for v_req in
    select elem
    from jsonb_array_elements(
  ...
  loop
    ...
    for v_field_citation in
      select elem
      from jsonb_array_elements(
        coalesce(v_req->'citations', '[]'::jsonb)
      ) as elem
    loop
      continue when nullif(trim(v_field_citation->>'source_url'), '') is null;

      v_access_date_raw := nullif(trim(v_field_citation->>'access_date'), '');
      if v_access_date_raw is not null
         and v_access_date_raw !~ '^\d{4}-\d{2}-\d{2}$'
      then
        raise exception
          'invalid requirement citations access_date "%" for source_url %',
          v_access_date_raw,
          trim(v_field_citation->>'source_url');
      end if;

      begin
        v_access_date := v_access_date_raw::date;
      exception
        when others then
          raise exception
            'invalid requirement citations access_date "%" for source_url %',
            coalesce(v_access_date_raw, ''),
            trim(v_field_citation->>'source_url');
      end;
      ...
      insert into public.content_citations (
        ...
        'requirement', v_req_id,
        trim(v_field_citation->>'source_url'),
        nullif(trim(v_field_citation->>'source_title'), ''),
        nullif(trim(v_field_citation->>'publisher'), ''),
        nullif(trim(v_field_citation->>'source_type'), ''),
        v_access_date,
        nullif(trim(v_field_citation->>'evidence_note'), ''),
        coalesce((v_field_citation->>'is_sourced_fact')::boolean, true),
        system_profile_id
      )
      on conflict (target_type, target_id, source_url) do nothing;
    end loop;
  end loop;

  -- Pilot framework: insert row then attach field-level citations.
  ...
  if v_pf is not null and v_pf <> 'null'::jsonb then
    ...
    loop
      ...
      then
        raise exception
          'invalid pilot_framework citations access_date "%" for source_url %',
          v_access_date_raw,
          trim(v_field_citation->>'source_url');
      end if;
  ...
  -- Problem-level citations (unchanged).
  for v_citation in
    select elem
    from jsonb_array_elements(
  ...
  loop
    ...
    v_access_date_raw := nullif(trim(v_citation->>'access_date'), '');
    if v_access_date_raw is not null and v_access_date_raw !~ '^\d{4}-\d{2}-\d{2}$' then
      raise exception
