-- ============================================================
-- 20260418002000_harden_content_citations.sql
--
-- Additional provenance hardening:
-- - prevent duplicate citations for the same target/source_url
-- - clean up citations automatically when supported parent rows are deleted
-- ============================================================

create unique index if not exists content_citations_target_source_url_uidx
  on public.content_citations (target_type, target_id, source_url);

create or replace function public.delete_content_citations_for_target()
returns trigger
language plpgsql
security definer
...
as $$
begin
  delete from public.content_citations
...
drop trigger if exists cleanup_problem_template_citations on public.problem_templates;
create trigger cleanup_problem_template_citations
  after delete on public.problem_templates
  for each row execute function public.delete_content_citations_for_target('problem_template');

drop trigger if exists cleanup_requirement_citations on public.requirements;
create trigger cleanup_requirement_citations
  after delete on public.requirements
  for each row execute function public.delete_content_citations_for_target('requirement');

drop trigger if exists cleanup_solution_approach_citations on public.solution_approaches;
create trigger cleanup_solution_approach_citations
  after delete on public.solution_approaches
  for each row execute function public.delete_content_citations_for_target('solution_approach');
