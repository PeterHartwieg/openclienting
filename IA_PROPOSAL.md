...
- **Problem detail active highlight** (proposal §F). Verified end-to-end — `isNavItemActive` already uses `startsWith` matching on `/problems`, so `/problems/[id]` correctly highlights the Problems nav item. No code change needed. Verified in commit `303f8c4` session.
- **Mobile drawer — task-flow-first redesign** (proposal §B). Drawer now renders three blocks: (1) "Jump back in" recent items (top 3), (2) primary tasks (Submit, My submissions, Notifications, moderator-only Review queue deep-linked to highest-count queue), (3) full category nav with collapsible groups and active-group auto-expansion. `WorkspaceNav` gained a `variant: "desktop" | "mobile-collapsed"` prop — desktop path byte-for-byte identical. Shipped in commits `6e93295` + integration `9667c04`.
- **Phase 4 productivity — global search + recent items** (scoped slice). Global problem search in workspace top bar backed by a `search_problems(q, lim)` Postgres RPC (SECURITY INVOKER, published-only via RLS), with debounced combobox + keyboard nav. Client-side recent-nav via `useRecentNav` hook (localStorage `oc:recent-nav:v1`, max 10), surfaced in a "Jump back in" dashboard card and the mobile drawer. Saved filters and command palette explicitly deferred pending GA4 baseline data. Shipped in commit `5c0ec7b` + integration `9667c04`.
- **Phase 5 measurement — IA analytics events.** `src/lib/analytics/ia-events.ts` ships a typed dispatcher for four events: `ia_nav_click` (section/shell/surface), `ia_first_action_after_login`, `ia_dashboard_card_click`, `ia_moderation_queue_open`. Consent-gated via the existing `oc_cookie_consent` key. GTM tags configured to forward all four to GA4 as custom dimensions (IA Section/Shell/Surface/Card/Queue). Memory pointer: [reference_ia_analytics.md]. Shipped in commit `74c175f`.
- **Moderation layer cleanup.** Removed redundant `requireRole("moderator")` calls from `moderate/problems/[id]`, `moderate/history`, `moderate/tags` — the `(shell)/moderate/layout.tsx` enforces the role at the layout level. Shipped in commit `303f8c4`.
...
All originally-scoped phases (1–5) are done. Two items from Phase 4 are explicitly deferred pending GA4 data from the Phase 5 baseline:

1. **Saved filters** (proposal §E). Deferred from the filter-context slice. Revisit once GA4 shows enough return visits to the problems list with identical filter combinations to justify the build cost.
2. **Command palette.** Deferred from the Phase 4 productivity slice. Revisit once GA4 `ia_nav_click` data shows heavy nav usage that a palette would actually shortcut.
